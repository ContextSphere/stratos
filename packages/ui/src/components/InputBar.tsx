...
export const InputBar = forwardRef<InputBarRef, Props>(function InputBar(
  ...
  const [slashMenu, setSlashMenu] = useState<{ triggerPos: number } | null>(null)
  ...
  const handleInput = useCallback(() => {
    ...
    if (slashCommands.length > 0) {
      ...
      if (lastSlashIdx >= 0 && (lastSlashIdx === 0 || /\s/.test(textBefore[lastSlashIdx - 1]))) {
  ...
  const canSend = (hasContent || images.length > 0) && (!isStreaming || isInteractive)

  const placeholder = useMemo(() => {
    if (isDragging) return 'Drop image...'
    if (interactiveMode?.type === 'plan-review') return 'Provide feedback to revise the plan...'
  ...
  return (
    <div
      ...
      className={`relative flex-shrink-0 border-t transition-colors ${
        isDragging ? 'border-blue-500 bg-blue-950/20' : 'border-[#2a2a2a] bg-[#0f0f0f]'
      } p-4`}
    ...
    >
      {slashMenu !== null && slashCommands.length > 0 && (
        <SlashCommandMenu
          ...
          position={{ bottom: containerRef.current ? containerRef.current.offsetHeight : 60, left: 16 }}
      ...
      <div>
        {isDragging && (
          <div className="mb-2 text-center text-blue-400 text-xs py-1">Drop image here</div>
        )}
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="no-drag flex-shrink-0 w-10 h-10 rounded-xl bg-[#1a1a1a] border border-[#333] hover:bg-[#2a2a2a] text-gray-400 hover:text-gray-200 flex items-center justify-center transition-colors"
            title="Attach image"
          >
          ...
          <div
            ...
            className="no-drag flex-1 min-h-[44px] max-h-[200px] overflow-y-auto bg-[#1a1a1a] border border-[#333] rounded-xl px-4 py-3 text-sm text-gray-200 focus:outline-none focus:border-blue-500 transition-colors empty:before:content-[attr(data-placeholder)] empty:before:text-gray-600 empty:before:pointer-events-none whitespace-pre-wrap"
            role="textbox"
            aria-multiline="true"
          />
