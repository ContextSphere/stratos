...
function truncateText(text: string, lines: number): { truncated: string; hasMore: boolean } {
...
export function TaskCard({ taskInfo, childToolCalls, onToggleToolCalls }: Props): React.ReactElement {
  const [detailsExpanded, setDetailsExpanded] = useState(false)
  const [promptExpanded, setPromptExpanded] = useState(false)
  const [resultExpanded, setResultExpanded] = useState(false)

  const completedCount = childToolCalls.filter(tc => tc.status === 'completed').length
  ...
  const { truncated: promptPreview, hasMore: hasMorePrompt } = truncateText(taskInfo.prompt, 3)
    ...
    <div className="rounded-lg bg-[#0d0d18] border border-[#3a3a5a] p-3 text-xs">
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-gray-200">{taskInfo.subagentType}</span>
        </div>
        <span className={`text-xs font-medium ${statusColors[taskInfo.status]}`}>
          {statusLabels[taskInfo.status]}
          {taskInfo.status === 'running' && <span className="ml-1 animate-pulse">⟳</span>}
        </span>
      </div>
        ...
        {detailsExpanded && (
          <div className="mt-2 pl-4 space-y-2">
            <div>
              ...
              <pre className="p-2 bg-[#0a0a0a] rounded text-gray-300 font-mono text-xs whitespace-pre-wrap">
      ...
      {totalCount > 0 && (
        <div className="mt-2">
          <button
            ...
            className="flex items-center gap-1.5 text-gray-400 hover:text-gray-300 transition-colors"
          >
            <span className="text-gray-600">{toolCallsExpanded ? '▾' : '▸'}</span>
        ...
        <div className="mt-2">
          ...
          {resultExpanded && (
            <div className="mt-2 pl-4">
              <pre className="p-2 bg-[#0a0a0a] rounded text-gray-300 font-mono text-xs whitespace-pre-wrap max-h-64 overflow-y-auto">
                {resultText}
              </pre>
            </div>
          )}
