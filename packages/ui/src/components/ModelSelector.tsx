...
function isOpusModel(model: ModelInfo | undefined): boolean {
  ...
  const text = `${model.value} ${model.displayName} ${model.description}`.toLowerCase()
...
export default function ModelSelector({
  ...
  useEffect(() => {
    if (isOpen) {
      const idx = models.findIndex((m) => m.value === (selectedModel || models[0]?.value))
  ...
  const showEffort = isOpusModel(currentModelInfo)
            ...
            {models.map((model, i) => (
              <button
                ...
                className={`w-full text-left px-3 py-2 text-xs flex flex-col gap-0.5 ${
                  i === highlightedIndex ? 'bg-[#2a2a2a] text-gray-200' : 'text-gray-400 hover:bg-[#222]'
                }`}
              >
                <span>{model.displayName}</span>
                {model.description && (
                  <span className="text-gray-600 text-[11px]">{model.description}</span>
                )}
              </button>
            ))}
          </div>
