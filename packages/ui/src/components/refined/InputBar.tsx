...
export const InputBar = forwardRef<InputBarRef, Props>(function InputBar(
  ...
  return (
    <div
      ref={containerRef}
      className={`relative flex-shrink-0 transition-colors ${classic ? "border-t border-[var(--border)] p-4" : "px-4 pb-4 pt-2"} ${
    ...
    >
      ...
      <div
        className={`flex w-full flex-col gap-2 ${classic ? "" : "mx-auto max-w-[720px]"}`}
      >
        <PendingMessages
        ...
        <div
          className={`overflow-visible transition-colors ${classic ? "grid grid-cols-[40px_minmax(0,1fr)_auto] items-center gap-x-2 border-0 bg-transparent" : "rounded-[14px] border bg-[var(--bg-surface)]"} ${
            isDragging
        ...
        >
          {(images.length > 0 || fileAttachments.length > 0) && (
            <div
              className={`flex flex-wrap gap-2 px-3 pt-3 ${classic ? "col-span-3 row-start-1 mb-2" : ""}`}
            >
              {images.map((img) => (
          ...
          {sendError && (
            <p
              role="alert"
              className={`px-3 pt-2 text-xs text-[var(--text-danger)] ${classic ? "col-span-3 row-start-4" : ""}`}
            >
              {sendError}
            </p>
          )}
          ...
          <div
            ...
            className={`no-drag w-full overflow-y-auto text-[var(--text-primary)] empty:before:pointer-events-none empty:before:content-[attr(data-placeholder)] empty:before:text-[var(--text-muted)] whitespace-pre-wrap ${classic ? "col-start-2 row-start-2 min-h-[44px] max-h-[200px] rounded-xl border border-[var(--border-mid)] bg-[var(--bg-surface)] px-4 py-3 text-sm focus:outline-none focus:border-blue-500" : "min-h-[70px] max-h-[220px] bg-transparent px-4 pb-2 pt-3.5 text-[15px] leading-6 focus:outline-none"}`}
            role="textbox"
            aria-multiline="true"
          />
          ...
          <div
            className={
              classic
                ? "contents"
                : "flex items-center justify-between gap-3 min-h-11 px-2.5 pb-2 pt-1"
            }
          >
            <div
              className={
                classic ? "contents" : "flex min-w-0 items-center gap-1.5"
              }
            >
              <button
                onClick={() => fileInputRef.current?.click()}
                className={`no-drag flex flex-shrink-0 items-center justify-center transition-colors ${classic ? "col-start-1 row-start-2 h-10 w-10 rounded-xl border border-[var(--border-mid)] bg-[var(--bg-surface)] text-[var(--text-muted)] hover:bg-[var(--border)] hover:text-[var(--text-primary)]" : "h-8 w-8 rounded-md text-[var(--text-secondary)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-primary)] focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[var(--text-muted)]"}`}
                title="Attach file"
              ...
              <div
                className={`flex min-w-0 items-center gap-1.5 ${classic ? "col-span-3 row-start-3 mt-2 flex-wrap" : ""}`}
              >
                {toolbar}
              </div>
            </div>
            ...
            <div
              className={`flex flex-shrink-0 items-center gap-1.5 ${classic ? "col-start-3 row-start-2" : ""}`}
            >
              ...
              <button
                ...
                disabled={!canSend}
                className={`no-drag flex flex-shrink-0 items-center justify-center gap-1.5 disabled:cursor-not-allowed ${classic ? "h-10 rounded-xl bg-blue-600 text-white hover:bg-blue-500 disabled:bg-[var(--border-mid)]" : "h-8 rounded-md bg-[var(--text-primary)] text-[var(--bg-main)] transition-opacity hover:opacity-85 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[var(--text-muted)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--bg-surface)] disabled:bg-[var(--border-mid)] disabled:text-[var(--text-muted)]"} ${
                  isStreaming && !isInteractive
                    ? classic
                      ? "px-3"
                      : "px-2.5"
