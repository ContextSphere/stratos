...
function ThinkingBlock({
  ...
  return (
    <div className="mb-1">
      ...
      {expanded && (
        <pre className="mt-1 max-h-64 overflow-y-auto whitespace-pre-wrap break-words rounded-lg bg-[var(--bg-overlay)] px-3 py-2 font-sans text-xs leading-relaxed text-[var(--text-muted)]">
          {content}
        </pre>
      )}
    </div>
          ...
          <div className="min-w-0 flex-1">
            ...
            {tn.outputFile && (
              <div className="mt-1">
                {onViewFile ? (
                  <button
                    onClick={() => onViewFile(tn.outputFile!)}
                    className="text-[var(--text-muted)] hover:text-[var(--text-secondary)] underline font-mono text-[11px]"
                  >
                    {tn.outputFile}
                  </button>
                ) : (
                  <span
                    className="text-[var(--text-muted)] font-mono text-[11px]"
                    title={tn.outputFile}
                  >
                    {tn.outputFile}
                  </span>
              ...
              <div className="flex flex-wrap gap-1.5 mb-2">
                {attachments.map((att, i) => (
                  <a
                    ...
                    className="flex items-center gap-1.5 rounded-lg border border-[var(--border-mid)] bg-[var(--bg-overlay)] px-2 py-1 text-xs text-[var(--text-secondary)] no-underline transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-primary)] cursor-pointer"
                    title={att.url}
                  >
                    <svg
              ...
              <div className="flex flex-wrap gap-2 mb-2">
                {message.images.map((img) => (
                  <img
                    ...
                    alt={img.name}
                    title={img.name}
                    className="max-w-[200px] max-h-[200px] rounded-lg object-cover border border-[var(--border-mid)]"
                  />
                ))}
              </div>
            )}
              ...
              message.fileAttachments.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-2">
                  {message.fileAttachments.map((file) => (
                    <div
                      key={file.id}
                      className="flex items-center gap-1.5 px-2 py-1 rounded-lg border border-[var(--border-mid)] bg-[var(--bg-overlay)] text-[var(--text-secondary)] text-xs max-w-[240px]"
                      title={file.path}
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
