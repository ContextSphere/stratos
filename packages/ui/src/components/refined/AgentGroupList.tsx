  ...
  const renderAgentRow = (agent: AgentDefinition) => {
    ...
    return (
      <div key={agent.id}>
        {/* Agent row */}
        <div
          className={`group no-drag relative flex h-[30px] items-center gap-2 rounded-md px-2 text-[13px] cursor-pointer transition-colors ${
            isActiveAgent
        ...
        {!isCollapsed && (
          <div className="space-y-0.5 mt-0.5">
            {agentThreads.length === 0 ? (
              <p className="py-1 pl-7 pr-3 text-[12px] text-[var(--text-muted)]">
                No threads
              </p>
            ) : (
