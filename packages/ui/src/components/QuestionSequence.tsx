  ...
  return (
    <div className="mt-3 space-y-2">
      ...
      {questions.slice(0, currentIndex).map((q, i) => (
        <div key={i} className="rounded-lg border border-[#2a2a2a] bg-[#111]/60 px-3 py-2 opacity-60">
      ...
      {completed &&
        questions.slice(currentIndex).map((q, i) => (
          <div
            key={currentIndex + i}
            className="rounded-lg border border-[#2a2a2a] bg-[#111]/60 px-3 py-2 opacity-60"
          >
            <p className="text-xs text-gray-400">{q.question}</p>
            <p className="text-xs text-blue-400 mt-0.5">{answers[q.question]}</p>
          </div>
