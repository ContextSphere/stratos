...
describe('ModeToggle', () => {
  ...
  it('shows description as title attribute', () => {
    render(<ModeToggle mode="default" onModeChange={onModeChange} />)
    expect(screen.getByTitle('Read-only. Plans without modifying files.')).toBeInTheDocument()
    expect(screen.getByTitle('Skips all permission prompts.')).toBeInTheDocument()
  })
})
