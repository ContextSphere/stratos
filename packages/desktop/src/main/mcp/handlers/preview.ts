...
export function createPreviewHandlers(deps: PreviewDeps): HandlerDef[] {
  ...
  return [
    defineHandler({
      ...
      description:
        "Open a file in the Stratos side preview pane. Markdown files (.md, .markdown) render as formatted text; images (.png, .jpg, .svg, .gif, .webp, .bmp, .ico, .tiff) show in an image viewer; PDFs and Office documents (.pptx, .ppt, .docx, .doc, .xlsx, .xls, .odp, .odt, .ods) render as a one-page-at-a-time gallery (Office formats require LibreOffice/soffice on PATH); all other files open in a code editor. Always use absolute file paths.",
      inputSchema: {
      ...
      handler: async (args) => {
        ...
        if (PDF_EXTENSIONS.has(ext) || OFFICE_EXTENSIONS.has(ext)) {
          let pages: string[];
          try {
            pages = await getPdfPagesForFile(args.file_path);
          } catch (err) {
            const msg = err instanceof Error ? err.message : String(err);
            return textResult(`Cannot render document: ${msg}`, true);
          }
