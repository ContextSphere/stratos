...
import type { ChatMessage, ToolCall, TaskInfo, PermissionRequest, AskUserQuestionRequest, PlanReviewRequest, ImageAttachment, TodoData, WorktreeProgressData } from '@agentpanel/ui'
...
interface UseChatReturn {
  ...
  sendMessage: (prompt: string, threadId?: string, images?: ImageAttachment[]) => Promise<void>
  ...
  respondPlanReview: (requestId: string, decision: { type: string; feedback?: string }) => void
...
function fromStoredMessage(m: StoredMessage): ChatMessage {
  return {
    ...
    ...(m.questionData ? { questionData: m.questionData as AskUserQuestionRequest } : {}),
    ...(m.questionAnswered && { questionAnswered: m.questionAnswered }),
    ...(m.planReviewData ? { planReviewData: m.planReviewData as PlanReviewRequest } : {}),
    ...(m.todoData ? { todoData: m.todoData as TodoData } : {}),
    ...(m.worktreeProgress ? { worktreeProgress: m.worktreeProgress as WorktreeProgressData } : {}),
    ...(m.images && m.images.length > 0 && { images: m.images })
  }
}
...
export function useChat(activeThreadId: string | null, options?: UseChatOptions): UseChatReturn {
  ...
  const [threadNotifications, setThreadNotifications] = useState<Map<string, string>>(new Map())
  const [interactiveMode, setInteractiveMode] = useState<InteractiveMode>({ type: 'none' })
  const [sessionStats, setSessionStats] = useState<SessionStats>({
    totalCost: 0, totalInputTokens: 0, totalOutputTokens: 0, contextWindow: null
  })
  ...
  const isStreaming = activeThreadId ? runningThreadIds.includes(activeThreadId) : false

  // Only show permission requests for the active thread
  const permissionRequest = permissionRequests.find(
  ...
  const computeSessionStats = useCallback((msgs: ChatMessage[]): SessionStats => {
    let totalCost = 0
  ...
  useEffect(() => {
    ...
    if (activeThreadId) {
      ...
      loadingThreadIdRef.current = null
      setMessages([])
      setSessionStats({ totalCost: 0, totalInputTokens: 0, totalOutputTokens: 0, contextWindow: null })
      setSessionTools(null)
    }
  }, [activeThreadId]) // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => {
    if (!activeThreadId || messages.length === 0 || loadingThreadIdRef.current) return
  ...
  useEffect(() => {
    ...
    api.onStreamMessage((data: unknown, threadId: string | null) => {
      ...
      switch (msg.type) {
        ...
        case 'tool_use': {
          ...
          if (msg.toolName === 'Task') {
            ...
            const taskInfo: TaskInfo = {
              ...
              subagentType: (msg.input.subagent_type as string) ?? 'general-purpose',
              ...
              prev.map((m) => {
                if (m.taskInfo?.taskId === state.activeTaskId) {
                  return {
                    ...
                    taskInfo: {
                      ...m.taskInfo,
                      childToolCalls: [...m.taskInfo.childToolCalls, msg.toolCallId]
                    },
                    toolCalls: [
                      ...(m.toolCalls ?? []),
                      toolCall
        ...
        case 'todo_update': {
          ...
          if (lastMsg && lastMsg.role === 'assistant' && lastMsg.todoData && !lastMsg.content) {
        ...
        case 'worktree_progress': {
          ...
          if (lastMsg && lastMsg.role === 'assistant' && lastMsg.worktreeProgress && !lastMsg.content) {
        ...
        case 'result': {
            ...
            for (let i = prev.length - 1; i >= 0; i--) {
              if (prev[i].role === 'assistant') {
                ...
                updated[i] = {
                  ...
                  cost: msg.cost,
                  usage: msg.usage,
                  ...(msg.contextWindow != null && { contextWindow: msg.contextWindow }),
          ...
          if (activeThreadIdRef.current === threadId) {
            setSessionStats((prev) => ({
              ...
              totalInputTokens: prev.totalInputTokens + (msg.usage?.inputTokens ?? 0),
              totalOutputTokens: prev.totalOutputTokens + (msg.usage?.outputTokens ?? 0),
              contextWindow: msg.contextWindow ?? prev.contextWindow
            }))
          }

          // Generate a better title after the first response if still "New chat"
          window.api.threadsGet(threadId).then((thread) => {
            if (thread && thread.title === 'New chat') {
              // Find first user message
              const firstUserMsg = state.messages.find(m => m.role === 'user')
              if (firstUserMsg?.content) {
                const trimmed = firstUserMsg.content.trim()
                const title = trimmed ? (trimmed.length > 50 ? trimmed.slice(0, 50) + '...' : trimmed) : 'New chat'
                window.api.threadsUpdate(threadId, { title })
        ...
        case 'session_init':
          ...
          if (msg.tools && Array.isArray(msg.tools)) {
            ...
            // Persist sessionTools to thread metadata
            if (threadId) {
              window.api.threadsUpdate(threadId, { sessionTools: msg.tools }).catch(() => {})
            }
          }
          break
      }
    ...
    api.onToolPermission((data: unknown, threadId: string | null) => {
      ...
      if (threadId && threadId !== activeThreadIdRef.current) {
        setThreadNotifications(prev => new Map(prev).set(threadId, 'permission'))
      }
    })

    api.onAskUserQuestion((data: AskUserQuestionRequest, threadId: string | null) => {
      if (!threadId) return
      ...
      // Set notification for background threads
      if (threadId !== activeThreadIdRef.current) {
        setThreadNotifications(prev => new Map(prev).set(threadId, 'question'))
      }
    })

    api.onPlanReview((data: PlanReviewRequest, threadId: string | null) => {
      if (!threadId) return
      const state = streamingThreadsRef.current.get(threadId)
      if (!state) return
      ...
      // Set notification for background threads
      if (threadId !== activeThreadIdRef.current) {
        setThreadNotifications(prev => new Map(prev).set(threadId, 'plan_review'))
      }
    })
    ...
    api.onSlashCommands((commands: { name: string; description?: string }[]) => {
      setSlashCommands(commands)
    })

    api.onModeChanged((data: { mode: AgentMode }, threadId: string | null) => {
      if (!threadId) return
      const state = streamingThreadsRef.current.get(threadId)
      if (!state) return
  ...
  const sendMessage = useCallback(
    async (prompt: string, threadId?: string, images?: ImageAttachment[]) => {
      ...
      const ipcImages = images?.map((img) => ({ dataUrl: img.dataUrl, mimeType: img.mimeType }))
  ...
  const interrupt = useCallback(async (threadId?: string) => {
    const tid = (typeof threadId === 'string' ? threadId : undefined) ?? activeThreadIdRef.current
    if (!tid) return
      ...
      if (activeId) {
        ...
        if (state) {
          state.messages = state.messages.map((m) => ({
            ...m,
            toolCalls: m.toolCalls?.map((tc) =>
              tc.status === 'running' ? { ...tc, status: 'denied' as const } : tc
            )
          }))
        }
      }
  ...
  const respondQuestion = useCallback((requestId: string, answers: Record<string, string>) => {
    ...
    const markAnswered = (msgs: ChatMessage[]): ChatMessage[] =>
      msgs.map((m) =>
        m.questionData?.requestId === requestId ? { ...m, questionAnswered: true } : m
      )
  ...
  const respondPlanReview = useCallback((requestId: string, decision: { type: string; feedback?: string }) => {
  ...
  const handleInteractiveResponse = useCallback((text: string) => {
    if (interactiveMode.type === 'plan-review') {
      // Send feedback through plan review callback
      respondPlanReview(interactiveMode.requestId, { type: 'deny', feedback: text })
  ...
  const updateTaskExpanded = useCallback((messageId: string, expanded: boolean) => {
    const activeId = activeThreadIdRef.current
    if (!activeId) return
