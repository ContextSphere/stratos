...
export function appendTraceEntry(
  ...
  try {
    ...
    console.error(
      `Failed to append trace entry for thread ${threadId}:`,
      error,
    );
  }
}

export function readTraceEntries(
  threadId: string,
  baseDir = DEFAULT_BASE_DIR,
): TraceEntry[] {
  const tracePath = getTracePath(threadId, baseDir);
  ...
  try {
    ...
    console.error(
      `Failed to read trace entries for thread ${threadId}:`,
      error,
    );
    return [];
  }
...
export function clearTraceFile(
  threadId: string,
  baseDir = DEFAULT_BASE_DIR,
): void {
  const tracePath = getTracePath(threadId, baseDir);
  if (existsSync(tracePath)) {
    try {
      ...
      console.error(
        `Failed to clear trace file for thread ${threadId}:`,
        error,
      );
    }
  }
