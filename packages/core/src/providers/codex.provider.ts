 ...
 */
function mapModeToPolicy(mode?: string, configSandbox?: string): {
  ...
  switch (mode) {
    ...
    case 'default':
      return { approvalPolicy: 'untrusted', sandbox: configSandbox ?? 'workspace-write' }
    case 'acceptEdits':
      return { approvalPolicy: 'on-request', sandbox: configSandbox ?? 'workspace-write' }
...
export class CodexProvider implements AgentProvider {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private notificationQueue: Array<{ method: string; params: any; id?: number }> = []
  ...
  private reasoningBuffers = new Map<string, { summary: string; content: string }>()
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private async waitForNotification(): Promise<{ method: string; params: any; id?: number } | null> {
  ...
  async *sendMessage(params: SendMessageParams): AsyncGenerator<AgentMessage> {
    ...
    const { approvalPolicy, sandbox } = mapModeToPolicy(mode, this.config.sandboxPolicy)
    const effort = mapThinkingEffort(params.thinkingEffort)

    // Determine if we should resume or start a new thread
    const shouldResume = params.sessionId && this.threadId && params.sessionId === this.threadId
    ...
    if (!this.threadId) {
      ...
      try {
        ...
        if (this.config.systemPrompt) {
          ...
          } else if (this.config.systemPrompt.type === 'preset' && this.config.systemPrompt.append) {
            threadStartParams.developerInstructions = this.config.systemPrompt.append
          }
        }
   ...
   */
  private async *processTurnNotifications(params: SendMessageParams): AsyncGenerator<AgentMessage> {
    ...
    while (!turnComplete) {
      ...
      switch (method) {
        ...
        case 'item/reasoning/summaryTextDelta': {
          ...
          if (delta) {
            const buf = this.reasoningBuffers.get(itemId) ?? { summary: '', content: '' }
            buf.summary += delta
        ...
        case 'item/completed': {
          const item = p.item ?? p
          yield* this.handleItemCompleted(item, streamingText, streamingReasoning)
        ...
        case 'turn/completed': {
          ...
          yield {
            ...
            stop_reason: status === 'completed' ? 'end_turn'
              : status === 'interrupted' ? 'stop_sequence'
                : status === 'failed' ? 'end_turn'
                  : 'end_turn'
          }
          break
        }
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private *handleItemCompleted(item: any, streamingText: string, streamingReasoning: string): Generator<AgentMessage> {
    const itemType = item?.type
    ...
    switch (itemType) {
      ...
      case 'commandExecution': {
        ...
        yield {
          type: 'tool_result',
          toolCallId,
          output: outputParts.join('') || (item.status === 'completed' ? '(completed)' : `(${item.status})`)
        }
      ...
      case 'fileChange': {
        ...
        const streamedDiff = itemId ? this.fileChangeOutputBuffers.get(itemId) : undefined

        for (const change of changes) {
          const toolCallId = `file_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`
          const toolName = change.kind === 'add' ? 'Write'
            : change.kind === 'delete' ? 'Delete'
              : 'Edit'
          yield {
            type: 'tool_use',
            toolName,
      ...
      case 'mcpToolCall': {
        ...
        yield { type: 'tool_result', toolCallId, output: (output || `(${item.status})`) + durationSuffix }
        break
      }
      ...
      case 'dynamicToolCall': {
        ...
        const success = item.success === true ? '(success)' : item.success === false ? '(failed)' : `(${item.status})`
      ...
      case 'imageView': {
        ...
        yield { type: 'tool_result', toolCallId, output: `Viewed image: ${item.path ?? ''}` }
        break
      }
      ...
      case 'imageGeneration': {
        ...
        yield { type: 'tool_result', toolCallId, output: item.result ?? `(${item.status})` }
        break
      }
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private async *handleCommandApproval(p: any, requestId: number, params: SendMessageParams): AsyncGenerator<AgentMessage> {
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private async *handleFileChangeApproval(p: any, requestId: number, params: SendMessageParams): AsyncGenerator<AgentMessage> {
    const reason = p.reason ?? undefined
  ...
  async getAvailableModels(): Promise<ModelInfo[]> {
    ...
    try {
      ...
      const models: ModelInfo[] = (result?.data ?? []).map((m: {
        id: string
        model: string
        displayName: string
        description: string
    ...
    return [
      { value: 'gpt-5.3-codex', displayName: 'GPT-5.3 Codex', description: 'Latest frontier agentic coding model' },
      { value: 'gpt-5.2-codex', displayName: 'GPT-5.2 Codex', description: 'Advanced agentic coding model' },
      { value: 'gpt-5.1-codex', displayName: 'GPT-5.1 Codex', description: 'Agentic coding model with deep reasoning' },
      { value: 'gpt-5.1-codex-mini', displayName: 'GPT-5.1 Codex Mini', description: 'Fast, lightweight coding model' },
      { value: 'gpt-5.4', displayName: 'GPT-5.4', description: 'Latest general-purpose model' },
      { value: 'gpt-5.2', displayName: 'GPT-5.2', description: 'General-purpose model with coding capabilities' }
    ]
  }
    ...
    try {
      ...
      if (Array.isArray(skills)) {
        return skills.map((skill: { name?: string; description?: string }) => ({
          name: skill.name ? (skill.name.startsWith('/') ? skill.name : `/${skill.name}`) : '/unknown',
