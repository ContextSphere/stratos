...
The AgentPanel Electron app uses a custom icon and process name in the macOS dock. This required solving several platform-specific issues around icon transparency, sizing, process naming, and dev-mode detection.
...
ICONSET="/tmp/AgentPanel.iconset"
mkdir -p "$ICONSET"
...
1. Renames `Electron.app` to `AgentPanel.app`
2. Renames the executable `Electron` to `AgentPanel`
