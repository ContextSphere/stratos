...
Stratos supports **OpenAI Codex** as a second provider alongside Claude Code. Users can select which provider to use per-thread via the provider toggle in the bottom toolbar.
...
Note: Codex also supports `none` and `minimal` but Stratos doesn't expose these.
...
4. **Server requests vs notifications** — Approval requests are server-initiated _requests_ (have `id` + `method`) and require a JSON-RPC response. Regular streaming events are _notifications_ (only `method`, no `id`). Both arrive on the same stdout line stream.
