# Stratos — Open Source Design Doc
...
Stratos — an open-source platform to manage AI agent sessions with a polished UI. Built on Electron + React + Claude Agent SDK. By ContextSphere.

## What is Stratos vs ContextSphere?

|                  | **Stratos** (OSS)                               | **ContextSphere** (Proprietary)     |
| ---------------- | ----------------------------------------------- | ----------------------------------- |
...
| **Relationship** | The base platform                               | Built on top of Stratos             |
...
5. **How does ContextSphere consume Stratos?** Fork? Git subtree? npm package? Monorepo with private layer?
...
| --- | -------------------- | ------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| 1   | OSS project name     | **Stratos** (working name)                              | Control panel for agents, by ContextSphere                                                                        |
...
| 9   | External UI libs     | **Wrapped, never exposed**                              | Monaco, react-markdown, etc. are internal deps of `ui` — consumers use Stratos components, not raw libs           |
...
| 13  | Screen customization | **Slot-based layout + composable components**           | OSS provides `StratosLayout` with slots + raw components; CS can customize slots or build entirely custom screens |
...
| **Stratos**     | Control panel for agents (minor dead collision: 65 stars, inactive 2 yrs) |
...
**`@stratosapp/ui` — React Component Library**
...
- External UI libraries (Monaco, react-markdown, react-syntax-highlighter, react-resizable-panels) are internal dependencies — never exposed to consumers. Wrapped behind Stratos's own component API so internals can be swapped without breaking consumer code.

**`@stratosapp/core` — SDK & Provider Abstraction**
...
**`@stratosapp/desktop` — Electron App**
...
### How Different Consumers Use Stratos

```
Web developer:      @stratosapp/ui + @stratosapp/core → custom web app
Desktop user:       @stratosapp/desktop (batteries included)
SDK-only user:      @stratosapp/core → own UI, any framework
ContextSphere:      All three + KG + integrations (proprietary layer)
```

### Extensibility Model

Stratos provides primitives + sensible defaults. ContextSphere (or any consumer) injects its own implementations.
...
import { ChatView, ThreadList, ToolCallCard } from '@stratosapp/ui'
